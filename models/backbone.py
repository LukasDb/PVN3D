...
def get_backbone_model(
    ...
    get_func = Backbones.get_backbone
    try:
        backbone_func = inject_global_submodules(get_func)
        backbone = backbone_func(
            name=name, input_shape=input_shape, weights=weights, include_top=include_top
        )
    except Exception:
        backbone_func = get_func
        backbone = backbone_func(
            name=name, input_shape=input_shape, weights=weights, include_top=include_top
        )
    ...
    if downsample_factor == 16:
        ...
        raise ValueError(
            "Unsupported factor - `{}`, Use 4, 8, 16, 32.".format(downsample_factor)
        )

    input_ = backbone.input

    output = (
        backbone.get_layer(name=layer_idx).output
        if isinstance(layer_idx, str)
        else backbone.get_layer(index=layer_idx).output
    )

    model = tf.keras.Model(input_, output, name=name + "_backbone")
