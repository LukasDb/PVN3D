...
def match_choose_adp(prediction, choose, crop_down_factor, resnet_input_shape):
    ...
    bs = shape[0]
    c = shape[-1]
    prediction = tf.reshape(prediction, shape=(bs, -1, c))# [b, h*w, c]
    ...
    feats_inds = tf.reshape(feats_inds, shape=(bs, -1)) # [b, num_points, 1]
    pre_match = tf.gather(prediction, feats_inds, batch_dims=1)
    return pre_match
...
def map_indices_to_feature_map(indices, resnet_shape, image_shapes):
    ...
    """
    scales = resnet_shape[0] / image_shapes[:, 0][..., tf.newaxis]
    y_inds = tf.cast(tf.floor(tf.cast(indices // image_shapes[:, 1, tf.newaxis], tf.float32) * scales), tf.int32)
    x_inds = tf.cast(tf.floor(tf.cast(indices % image_shapes[:, 1, tf.newaxis], tf.float32) * scales), tf.int32)
    # to flattened inds
    return y_inds * resnet_shape[1] + x_inds
