...
class PvnLoss(tf.keras.losses.Loss):
    def __init__(
        self, *, binary_loss, kp_loss_discount, cp_loss_discount, seg_loss_discount
    ):
        super().__init__()
    ...
    def l1_loss(offset_pred, offset_gt, mask_labels):
        ...
        """
        bs, n_pts, n_kpts = tf.shape(offset_pred)[0], tf.shape(offset_pred)[1], tf.shape(offset_pred)[2]
    ...
    def call(self, y_true, y_pred):
        ...
        if self.binary_loss:
            loss_seg = self.BinaryFocalLoss(mask_selected, seg_pred)  # return batch-wise value
        else:
