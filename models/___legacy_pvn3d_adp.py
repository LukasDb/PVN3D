...
class Pvn3dNet(tf.keras.Model):
    ...
    def __init__(
        ...
        self.resnet_net = _ResNet(self.params.resnet_params, resnet_input_shape)
        self.resnet_model = self.resnet_net.build_resnet_model()

        self.psp_net = _PspNet(self.params.psp_params)
...
def forward_pass(input_data, model, training=False):
    ...
    inputs = [
        pcld,
        tf.cast(sampled_index, tf.int32),
        rgb,
        tf.cast(crop_factor, tf.int32),
    ]
