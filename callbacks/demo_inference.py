...
class DemoInference(cb.Callback):
    ...
    def on_epoch_end(self, epoch: int, logs=None):
        ...
        for (x,y) in self.demo_set:
            ...
            b_R, b_t, b_kpts_pred, (_, b_seg_pred, _, _, b_sampled_inds, _) = self.model(x, training=False)
            ...
            b_kpts_gt = tf.matmul(b_mesh_kpts, b_rt[:, :3, :3]) + b_rt[:, tf.newaxis, :3, 3] # [b, 9,3]
            ...
            b_kpts_gt = b_kpts_gt * tf.stack([cam_fx, cam_fy], axis=1)[:,tf.newaxis,:] + tf.stack([cam_cx, cam_cy], axis=1)[:,tf.newaxis,:] # [b, 9, 2]
            b_kpts_gt = b_kpts_gt.numpy().astype(int)

            b_kpts_pred = b_kpts_pred[:, :, :2] / b_kpts_pred[:, :, 2:] # [b, 9, 2]
            b_kpts_pred = b_kpts_pred * tf.stack([cam_fx, cam_fy], axis=1)[:,tf.newaxis,:] + tf.stack([cam_cx, cam_cy], axis=1)[:,tf.newaxis,:] # [b, 9, 2]
            b_kpts_pred = b_kpts_pred.numpy().astype(int)
            


            for rgb, roi, seg_pred, sampled_inds, kpts_gt, kpts_pred in zip(
                b_rgb, b_roi, b_seg_pred, b_sampled_inds, b_kpts_gt, b_kpts_pred):
                ...
                for (x1, y1), (x2, y2) in zip(kpts_gt, kpts_pred):
                    cv2.circle(rgb2, (x1, y1), 2, (0, 255, 0), -1)
                    cv2.circle(rgb2, (x2, y2), 2, (255, 0, 0), -1)
                    cv2.line(rgb2, (x1, y1), (x2, y2), (0, 0, 255), 1)
