...
class MlpClusterNet:
    ...
    def kp_cp_layers(self, rgbd_features):
        ...
        kp_cp_pre_feats = Conv1D(
            filters=self.num_kp_cp * self.channel_xyz,
            kernel_size=1,
            activation="relu",
            name="kp_cp_pre_feats",
        )(
            conv1d_3
        )  # [Bs, n_pts, num_kp_cp * channel_xyz]

        kp_cp_ofst = tf.reshape(
            kp_cp_pre_feats,
            shape=(-1, self.num_pts, self.num_kp_cp, self.channel_xyz),
            name="features reshape",
        )
    ...
    def points_candidates_filter(self, kp_cp_ofst, pcld_xyz, seg_pre, target_cls):
        ...
        kp_cp_cls_filtered = tf.multiply(
            kp_cp_candidates, segs_mask
        )  # [Bs, n_pts, n_kp_cp, 3]
        kp_cp_cls_trans = tf.transpose(
            kp_cp_cls_filtered, perm=(0, 2, 1, 3)
        )  # [Bs, num_kp_cp, n_pts, 3]

        kp_cp_cls_trans_reshape = tf.reshape(
            kp_cp_cls_trans, shape=(-1, self.num_kp_cp, self.num_pts * self.channel_xyz)
        )  # [Bs, num_kp_cp, n_pts * channel_xyz]
    ...
    def pts_voting_layer(self, kp_cp_cls_trans_reshape):
        ...
        """
        pts_vote_1 = Conv1D(
            self.params.pts_vote_dense_1_dim,
            kernel_size=1,
            activation="relu",
            name="points_voted_1",
        )(kp_cp_cls_trans_reshape)
        pts_vote_2 = Conv1D(
            self.params.pts_vote_dense_2_dim,
            kernel_size=1,
            activation="relu",
            name="points_voted_2",
        )(pts_vote_1)
        pts_voted = Conv1D(
            self.channel_xyz, kernel_size=1, activation=None, name="points_voted"
        )(
            pts_vote_2
        )  # [Bs, num_kp_cp, 3]
        return pts_voted
    ...
    def build_mlp_cluster_model(self, rgbd_features_shape):
        ...
        input_target_cls = tf.keras.Input(
            shape=self.params.target_cls_shape, name="target_cls_input", dtype=tf.int64
        )

        input_points_xyz = tf.keras.Input(
            shape=(self.num_pts, self.channel_xyz), name="pcld_xyz_input"
        )
        ...
        kp_cp_cls_trans_reshape = self.points_candidates_filter(
            kp_cp_ofst=kp_cp_ofst,
            pcld_xyz=input_points_xyz,
            seg_pre=sm_pre_feats,
            target_cls=input_target_cls,
        )

        pts_voted = self.pts_voting_layer(kp_cp_cls_trans_reshape)
        model = tf.keras.Model(
            inputs=[input_rgbd_features, input_target_cls, input_points_xyz],
            outputs=[kp_cp_ofst, sm_pre_feats, pts_voted],
            name="kpts_cluster_model",
        )
