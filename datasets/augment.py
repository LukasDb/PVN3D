...
class AugmentSettings:
    ...
    plane_elevation = 80.0 / 180 * np.pi
    ...
    downsample_and_upsample = (
        None  # 2 # simulate a lower resolution sensor by down- and upsampling
    )
...
class DepthAugmenter:
    """new class with similar functionality but improved performance"""

    @staticmethod
    def get_augment_function(h: int, w: int):
        w_res = list(get_possible_res_generator(w))
        h_res = list(get_possible_res_generator(h))

        def get_valid_perlin_res(freqency):
            ind_w = tf.cast(tf.round(freqency[1] * (len(w_res) - 1)), tf.int32)
            ind_h = tf.cast(tf.round(freqency[0] * (len(h_res) - 1)), tf.int32)
            return h_res[ind_h], w_res[ind_w]

        def get_random_perlin_res(shape):
            low_freq = np.random.normal(
                AugmentSettings.perlin_res_low,
                scale=AugmentSettings.perlin_frequency_std,
                size=(2,),
            )
            high_freq = np.random.normal(
                AugmentSettings.perlin_res_high,
                scale=AugmentSettings.perlin_frequency_std,
                size=(2,),
            )
            low_freq = np.clip(low_freq, 0.0, 1.0)
            high_freq = np.clip(high_freq, 0.0, 1.0)
        ...
        @tf.function
        def augment(*, rgb, depth, **kwargs):
            """
            depth and rgb : [b, h, w, c]
            """
            # always add depth background -> maybe not necessary since we have now background depth
            # depth = add_background_depth(depth[tf.newaxis, :, :])[0]  # add and remove batch
            ...
            depth_mask = depth > 1e-6

            if noise_dev_max is not None and noise_dev_min is not None:
                noise_dev = tf.random.uniform((1,), noise_dev_min, noise_dev_max)
                per_pixel_noise = tf.random.normal(tf.shape(depth), mean=0.0, stddev=noise_dev)
                per_pixel_noise = depth_mask * per_pixel_noise
                depth = depth + per_pixel_noise
                # depth = tf.where(depth > 1e-6, depth + per_pixel_noise, depth)

            h, w = tf.shape(depth)[1], tf.shape(depth)[2]
            ...
            depth = tf.where(perlin_noise < 0.6, depth, 0.0)

            if (
                AugmentSettings.edge_frequency is not None
                and AugmentSettings.edge_range is not None
            ):
            ...
            if AugmentSettings.camera_unit is not None:
                depth = (
                    tf.math.floordiv(depth, AugmentSettings.camera_unit)
                    * AugmentSettings.camera_unit
                )

            rgb = tf.cast(augment_rgb(tf.last(rgb, tf.float32) / 255.0) * 255, tf.uint8)

            return {"rgb": rgb, "depth": depth, **kwargs}
...
def add_background_depth(depth, obj_pos=None, camera_matrix=None, rgb2noise=None):
    ...
    __depth = tf.where(depth < 100.0, depth, 0.0)  # set nonexistent depth to 0
    __depth2 = tf.where(__depth > 0.01, depth, 100.0)
    ...
    fake_depth += (
        get_fake_depth(depth, method=None, tilted_plane=True) * plane_mid_z
    )  # adds tilted plane
    ...
    perlin_noise = tf.expand_dims(tf.expand_dims(perlin_noise, 0), -1)
    fake_depth = tf.where(
        perlin_noise < 0.7, fake_depth, fake_depth + tf.random.normal((), 0, 0.5)
    )

    if AugmentSettings.move_background_to_obj and obj_pos is not None:
        assert (
            len(obj_pos) == 3 and camera_matrix is not None
        ), "Specify object position and camera matrix!"
        z = obj_pos[2]  # z
    ...
    if AugmentSettings.rgb2noise_amplitude > 0:
        rgb2noise = (
            2 * (tf.math.reduce_mean(rgb2noise, -1) - 0.5) * AugmentSettings.rgb2noise_amplitude
        )
...
def augment_rgb(rgb):
    ...
    if p_sat > 0 and 0 in ops:
        rgb = tf.image.random_saturation(rgb, 1.0 - p_sat, 1.0 + p_sat)
    if p_hue > 0 and 1 in ops:
        rgb = tf.image.random_hue(rgb, p_hue)
    if p_contr > 0 and 2 in ops:
        rgb = tf.image.random_contrast(rgb, 1.0 - p_contr, 1.0 + p_contr)
    ...
    if p_noise > 0 and 4 in ops:
        ...
        noise = tf.random.normal(
            shape=tf.shape(rgb), mean=0.0, stddev=effective_p_noise, dtype=rgb.dtype
        )
        rgb += noise
    if p_perlin_per_channel > 0 and 5 in ops:
        rgb_size = tf.shape(rgb)[:2].numpy()
        p_amplitude = tf.random.uniform((6,), 0.0, p_perlin_per_channel)
    ...
    if AugmentSettings.blur_max > 0 and 6 in ops:
        ksize = tf.random.uniform(
            (), AugmentSettings.blur_min, AugmentSettings.blur_max, dtype=tf.int32
        )
...
def warp_edges(depth, shift_kernel="RECT"):
    ...
    depth_edges = tf.where(depth_edges > 0.008, 1.0, 0.0).numpy()
    ...
    dir_map *= len(neighbor_indices) / 2.0
    dir_map = np.floor(dir_map).astype(int)
    ...
    pixel_indices = np.where(
        pixel_indices < 0, pixel_indices + range * row_length, pixel_indices
    )  # shift negative indices into img
    pixel_indices = np.where(
        pixel_indices >= np.prod(depth_shape),
        pixel_indices - range * row_length - range,
        pixel_indices,
    )  # shift negative indices into img

    warped = (
        depth.numpy().flatten()[pixel_indices.flatten()].reshape(depth_shape)
    )  # replace pixels with chosen neighbor
...
def get_random_perlin_res(shape):
    low_freq = np.random.normal(
        AugmentSettings.perlin_res_low, scale=AugmentSettings.perlin_frequency_std, size=(2,)
    )
    high_freq = np.random.normal(
        AugmentSettings.perlin_res_high, scale=AugmentSettings.perlin_frequency_std, size=(2,)
    )
    low_freq = np.clip(low_freq, 0.0, 1.0)
    high_freq = np.clip(high_freq, 0.0, 1.0)
...
def perlin_noise_tf(shape, res, interpolant=smooth_interpolant):
    ...
    grid = tf.truncatemod(grid, 1.0)
    grid = tf.where(tf.abs(grid - 1.0) < 0.001, 0.0, tf.cast(grid, tf.float32))
    ...
    n10 = tf.math.reduce_sum(tf.stack((grid[:, :, 0] - 1.0, grid[:, :, 1]), -1) * g10, 2)
    n01 = tf.math.reduce_sum(tf.stack((grid[:, :, 0], grid[:, :, 1] - 1.0), -1) * g01, 2)
    n11 = tf.math.reduce_sum(tf.stack((grid[:, :, 0] - 1.0, grid[:, :, 1] - 1.0), -1) * g11, 2)
    ...
    return tf.math.sqrt(2.0) * ((1 - grid[:, :, 1]) * n0 + grid[:, :, 1] * n1)
...
def add_perlin_noise_to_depth(depth, res_low, res_high):
    ...
    perlin_noise += (
        0.5 * amplitude * perlin_noise_tf(dpt_shape, res_high)
    )  # ,  interpolant = jagged_interpolant)
...
def get_fake_depth(depth, method, tilted_plane=False):
    ...
    if tilted_plane:
        ...
        elevation = tf.random.uniform(
            (), -AugmentSettings.plane_elevation, AugmentSettings.plane_elevation
        )
        ...
        nodes = tf.expand_dims(
            tf.expand_dims([[top_left, top_right], [bottom_left, bottom_right]], 0), -1
        )
        method = "bilinear"

    img_size = tf.shape(depth)[1:3]
...
def rotate_datapoint(img_likes, Rt=None):
    ...
    for img in img_likes:
        if img is not None:
            ...
            if img.shape[-1] == 1:
                rot_img = np.expand_dims(rot_img, -1)  # re-add channel dim
